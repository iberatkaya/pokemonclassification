...
class App extends Component<Props, State> {
  ...
  onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    this.setState({ loading: true }, async () => {
      ...
      console.log(resjson);
      if (resjson.status === '1')
        this.setState({ image: data.url, loading: false, pred: resjson.data, status: resjson.status })
      else
        this.setState({ status: resjson.status, loading: false })
    ...
    return (
      <div>
        <Navbar bg="success" variant="dark" expand="lg">
          <Navbar.Brand style={{ color: '#eee' }} href="/">Pokemon Classification</Navbar.Brand>
